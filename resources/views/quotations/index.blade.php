                            ...
                            <tbody class="divide-y divide-white/10 bg-transparent">
                                @forelse ($quotations as $quotation)
                                    <tr class="hover:bg-white/5 transition duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">#{{ $quotation->id }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">
                                            {{ $quotation->client->name }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">
                                            {{ $quotation->issue_date->format('M d, Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('quotations.show', $quotation) }}"
                                                class="text-slate-400 hover:text-white mr-3 transition-colors">View</a>
                                            ...
                                            <form action="{{ route('quotations.destroy', $quotation) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="text-rose-400 hover:text-rose-300 transition-colors">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-24 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <h3 class="text-lg font-medium text-white">No quotations found</h3>
                                                <p class="text-sm text-slate-400 max-w-sm mt-1 mb-6">Create a new quotation
                                                    to get started.</p>
                                            </div>
                                        </td>
                                    </tr>
