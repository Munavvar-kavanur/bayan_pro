    ...
    <div class="min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Invoices</h1>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage your billing and payments.</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <!-- Total Volume -->
                <div class="glass p-5 rounded-xl shadow-lg hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                ...
                <div class="glass p-5 rounded-xl shadow-lg hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                Total Invoices</p>
                            <h3 class="text-2xl font-bold text-slate-800 dark:text-white mt-1">{{ $totalInvoices }}</h3>
                        </div>
                        <div class="p-2 bg-slate-100 dark:bg-white/5 rounded-lg text-slate-500 dark:text-slate-400">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
            <div class="glass shadow-xl rounded-2xl overflow-hidden">
                ...
                @if(session('success'))
                    <div
                        class="bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 p-4 text-sm font-medium border-b border-emerald-500/20">
                        {{ session('success') }}
                    </div>
                @endif
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-10 w-10 bg-indigo-50 dark:bg-indigo-500/20 rounded-lg flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-xs ring-1 ring-indigo-500/30">
                                                #{{ str_pad($invoice->id, 3, '0', STR_PAD_LEFT) }}
                                            </div>
                                            <div class="ml-4">
                                                <a href="{{ route('invoices.show', $invoice) }}"
                                                    class="text-sm font-bold text-slate-800 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                                    Invoice #{{ $invoice->id }}
                                                </a>
                                                @if($invoice->project)
                                                    <div class="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
                                                        <span
                                                            class="w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-slate-600"></span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div
                                                class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold ring-2 ring-white dark:ring-slate-800 shadow-lg shadow-indigo-500/20">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                        <div
                                            ...
                                            <form action="{{ route('invoices.destroy', $invoice) }}" method="POST"
                                                ...
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                                    class="p-1.5 hover:bg-rose-100 dark:hover:bg-rose-500/20 rounded-lg text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors"
                                                    title="Delete">
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-24 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-slate-800 dark:text-white">No invoices yet
                                            </h3>
                                            <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm mt-1 mb-6">Create
                                                your first invoice
                ...
                @if($invoices->hasPages())
                    <div class="px-6 py-4 border-t border-gray-200 dark:border-white/10 bg-slate-50 dark:bg-white/5">
                        {{ $invoices->links() }}
                    </div>
                @endif
            </div>
