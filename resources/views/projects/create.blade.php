        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="glass overflow-hidden rounded-xl">
                <div class="p-6 text-slate-100">
                    <form action="{{ route('projects.store') }}" method="POST" class="max-w-2xl mx-auto space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                class="mt-1 block w-full rounded-lg bg-slate-900/50 border-white/10 text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 placeholder-slate-500">{{ old('description') }}</textarea>
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="in_progress" {{ old('status') == 'in_progress' ? 'selected' : '' }}
                                        class="bg-slate-800">In Progress</option>
                                    <option value="on_hold" {{ old('status') == 'on_hold' ? 'selected' : '' }}
                                        class="bg-slate-800">On Hold</option>
